...
OHLCTooltip.defaultProps = {
    ...
    origin: [0, 0],
  children: (props, moreProps, itemsToDisplay) => {
        const { className, textFill, labelFill } = props;
        const { onClick, fontFamily, fontSize } = props;
        const { displayDate, open, high, low, close, volume, x, y } = itemsToDisplay;
    return (<g className={`react-stockcharts-tooltip-hover ${className}`}   transform={`translate(${ x }, ${ y })`} onClick={onClick}>
      <ToolTipText x={0} y={0}
